...
module.exports = function withReleaseSigning(config) {
  return withAppBuildGradle(config, (config) => {
    ...
    if (gradle === beforeSigning) {
      throw new Error('withReleaseSigning: could not find signingConfigs { debug { ... } } to extend');
    }
    ...
    if (gradle === beforeBuildTypes) {
      throw new Error('withReleaseSigning: could not find release buildType signingConfig to redirect');
    }
